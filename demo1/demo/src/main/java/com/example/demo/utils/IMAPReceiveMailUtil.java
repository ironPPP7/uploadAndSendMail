...
public class IMAPReceiveMailUtil {
	...
	OSSUtil ossUtil;
	@Resource
	private FundCheckLenderService fundCheckLenderService;
	 ...
	 * 接收邮件
	 */
	public Map<String, Object> receive(Date beginDate,Date endDate) throws Exception {
...
//		System.err.println(DateUtil.getLast(20));
		SearchTerm comparisonTermGe = new ReceivedDateTerm(ComparisonTerm.GE, beginDate);
		...
		System.err.println(search.length);
		Map<String, Object> map = new HashMap<>(3);
		if(search.length != 0){
			map = parseMessage(search);
		}
		map.put("length", search.length);
		// 释放资源
		folder.close(true);
		imapStore.close();
		return map;
	 ...
	 */
	public Map<String, Object> parseMessage(Message... messages) throws Exception {
		...
		for (int i = 0, count = messages.length; i < count; i++) {
			...
			if (isContainerAttachment) {
				// 保存附件到oss
				map.put("fileName", "测试20200528.xlsx");
				fundCheckLenderService.saveToOss(msg);
			}
